...
The LSB(rightmost) of the first entry of the FAT specifies the block size with the mapping as below:
{LSB : size in bytes} = {0:256; 1:512; 2:1024; 3:2048; 4:4096}
The MSB(leftmost) of the first entry of the FAT specifies the number of blocks that FAT region occupies.
The MSB should be ranged from 1-32 (numbers outside of this range will be considered an error).
FAT region size = block size * block number
...
typedef struct FATConfig {
    ...
    unsigned int FATRegionSize;
    unsigned int FATEntryNum;
    unsigned int dataRegionSize;
